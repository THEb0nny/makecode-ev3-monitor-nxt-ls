...
let animationUpdateTime = control.millis();
...
forever(function () {
    ...
    if (state == State.ShowValues && brick.buttonEnter.wasPressed()) { // Старт калибровки
        state = State.WaitWhite;
    } else if (state == State.WaitWhite && brick.buttonUp.wasPressed()) { // Сохранение белого
        for (let i = 0; i < 4; i++) {
            whiteRefRaw[i] = refRaw[i];
        }
        state = State.WaitBlack;
    } else if (state == State.WaitBlack && brick.buttonDown.wasPressed()) { // Сохранение чёрного
        for (let i = 0; i < 4; i++) {
            blackRefRaw[i] = refRaw[i];
            threshold[i] = Math.round((whiteRefRaw[i] + blackRefRaw[i]) / 2);
        }
        state = State.WaitCalibrationStart;
    } else if (state == State.WaitCalibrationStart && brick.buttonEnter.wasPressed()) { // Старт прохода
        whiteSamples = [[], [], [], []];
        blackSamples = [[], [], [], []];
        state = State.Calibration;
    } else if (state == State.Calibration) { // Калибровка
        if (brick.buttonEnter.wasPressed()) {
            ...
            for (let i = 0; i < 4; i++) { // Сбор семплов
                // WHITE
    ...
    // Новый проход
    if (state == State.Completed && brick.buttonUp.wasPressed()) {
        state = State.WaitCalibrationStart;
    }
    ...
    if (state == State.Completed && brick.buttonEnter.wasPressed()) {
        for (let i = 0; i < 4; i++) {
            ...
            // Обновляем calibration
            whiteRefRaw[i] = finalWhite[i];
            blackRefRaw[i] = finalBlack[i];
            threshold[i] = Math.round((finalWhite[i] + finalBlack[i]) / 2);
        }
    ...
    if (control.millis() - uiUpdateTime > 50) {
        ...
        if (state == State.ShowValues) {
            ...
            brick.printString("КАЛИБРОВКА" + animationFrames[animationFrame], 6);
            brick.printString("Водите робота по линии", 8);
            brick.printString("Нажмите ENTER для завершения", 9);
            if (control.millis() - animationUpdateTime > 200) {
                animationUpdateTime = control.millis();
                animationFrame++;
                if (animationFrame >= animationFrames.length) animationFrame = 0;
            }
            ...
            brick.printString("Итог за все проходы", 8);
            brick.printString("ВНИЗ для сохранения", 9);
            brick.printString("Б: " + finalWhite.join(", "), 11);
            brick.printString("Ч: " + finalBlack.join(", "), 12);
        } else if (state == State.SaveCompleted) {
            brick.printString("Данные сохранены", 7);
            brick.printString("в постоянную память", 8);
        }
    }
